...
public sealed class AiPromptManifestBuilder : IAiPromptManifestBuilder
{
        ...
        IEnumerable<AiPromptTopic> topics)
    {
        ...
        prompt.AppendLine("- Prefer the most concise documented construct that expresses the intent. Favor declarative top-level directives and summary forms - `foreach`/range, `max_send_iterations`, `retry N with backoff { }`, `pipe`, `define`/`call`, top-level `expect`, and `stash` columns - over manually unrolling repeated or near-identical request blocks or steps.");
        prompt.AppendLine("- Be economical: make the smallest change that satisfies the request and stop. Do not pad the request or your reply with extra steps, boilerplate, defensive scaffolding, or restated context the user did not ask for.");
        prompt.AppendLine("- Treat a successful workspace mutation (such as creating a new script) as completing the turn. Do not redo it or also edit the active request unless the user asked for both.");
